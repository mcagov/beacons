...
describe("NotesGateway", () => {
  ...
  describe("updateNote", () => {
    ...
    it("should make a PATCH request to the note endpoint with the specific ID", async () => {
      ...
      expect(mockedAxios.patch).toHaveBeenCalledWith(
        `${applicationConfig.apiUrl}/note/${noteId}`,
        expectedPayload,
        config,
      );
    });
    ...
    it("should re-throw errors if the PATCH request fails", async () => {
      mockedAxios.patch.mockRejectedValue(new Error("500 Server Error"));

      await expect(notesGateway.updateNote(noteId, updateData)).rejects.toThrow(
        "500 Server Error",
      );
    });
  });
  ...
  describe("deleteNote", () => {
    ...
    it("should re-throw errors if the delete fails", async () => {

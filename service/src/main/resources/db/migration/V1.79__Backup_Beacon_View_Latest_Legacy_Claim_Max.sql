...
FROM (SELECT b.id,
      ...
      FROM legacy_beacon
               LEFT JOIN (SELECT legacy_beacon_id, MAX(when_happened) AS max_when_happened
                          FROM legacy_beacon_claim_event
                          GROUP BY legacy_beacon_id) c ON legacy_beacon.id = c.legacy_beacon_id
      WHERE c.when_happened IS NULL
         OR c.when_happened = legacy_beacon_claim_event.max_when_happened) AS beacon_backup

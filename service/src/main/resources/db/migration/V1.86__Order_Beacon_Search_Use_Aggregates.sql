...
UNION ALL
    ...
    FROM beacon b
         ...
         LEFT JOIN ( SELECT beacon_use.beacon_id,
                            replace(string_agg(beacon_use.activity, ', '::text ORDER BY beacon_use.main_use DESC, beacon_use.created_date, beacon_use.id), '_'::text, ' '::text) AS use_activities,
                            string_agg(beacon_use.vessel_name, ', '::text ORDER BY beacon_use.main_use DESC, beacon_use.created_date, beacon_use.id) AS vessel_names,
                            string_agg(beacon_use.registration_mark, ', '::text ORDER BY beacon_use.main_use DESC, beacon_use.created_date, beacon_use.id) AS registration_marks,
                            COALESCE(
